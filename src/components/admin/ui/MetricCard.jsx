...
export default function MetricCard({
  ...
  sparkline,
  isMoney = false,
  /** One-line definition of the metric, shown on hover (see ADMIN_METRIC_DEFINITIONS). */
  hint,
  /** Secondary fact under the value, e.g. contracted-but-unpaid MRR. */
  footnote,
  unavailable = false,
  unavailableReason,
}) {
  ...
  return (
    <article className="rounded-2xl border border-border/80 bg-card p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
      <div className="flex items-start justify-between gap-3">
        <p className="text-[13px] font-medium text-muted-foreground" title={hint || undefined}>
          {title}
          {hint ? <span className="ml-1 cursor-help text-muted-foreground/60" aria-hidden>ⓘ</span> : null}
        </p>
      ...
      {footnote && !unavailable ? (
        <p className="mt-1 text-[11px] leading-4 text-muted-foreground">{footnote}</p>
      ) : null}
    </article>
  );
}
