...
export default function StatCard({ title, value, change, icon: Icon }) {
  ...
  return (
    <div className="group min-w-0 rounded-xl border border-border bg-card px-4 py-3.5 shadow-sm interactive-card max-[359px]:px-3 max-[359px]:py-3 sm:px-5 sm:py-4">
      <div className="mb-2 flex min-w-0 items-start justify-between gap-2">
        <p className="min-w-0 flex-1 text-[11px] text-muted-foreground max-[359px]:text-[10px]">{title}</p>
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary transition-all duration-200 group-hover:bg-primary/15 group-hover:scale-110 max-[359px]:h-7 max-[359px]:w-7">
