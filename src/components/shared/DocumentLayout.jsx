...
export default function DocumentLayout({ user, title, documentNumber, date, children, downloadUrl, autoDownload = false }) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-background py-4 sm:py-8 print:bg-white print:py-0">
                <div className="w-full max-w-4xl mx-auto px-2 sm:px-4">
                    ...
                    <div className="print-container bg-card shadow-lg rounded-lg p-4 sm:p-8 print:bg-white print:shadow-none print:rounded-none overflow-x-auto">
                        {/* Header */}
                            ...
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                                <div>
                                    ...
                                    {user?.company_address && (
                                        <p className="text-muted-foreground mt-2 whitespace-pre-line text-xs sm:text-sm">{user.company_address}</p>
                                    )}
                                </div>
                                <div className="text-left sm:text-right">
                                    <h2 className="text-2xl sm:text-3xl font-bold text-[#f24e00] mb-2">{title}</h2>
                                    {documentNumber && <p className="text-muted-foreground text-sm"><strong>#:</strong> {documentNumber}</p>}
                                    {date && <p className="text-muted-foreground text-sm"><strong>Date:</strong> {date}</p>}
                                </div>
                            </div>
                        </header>
