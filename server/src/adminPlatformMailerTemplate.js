 ...
 * - ADMIN_OUTREACH_MAILER_HEADER_SUBTITLE — line under “Paidly” in header (default: “A quick note from Paidly”).
 * - ADMIN_OUTREACH_MAILER_FOOTER — extra footer banner (plain text, multiline); URLs linkified like the body.
 * - PUBLIC_APP_ORIGIN or CLIENT_ORIGIN (first comma-separated origin) — “Open Paidly” + /Login, footer links, and default header logo URL.
 * - ADMIN_OUTREACH_LOGO_URL — optional absolute https URL for the header mark (default: {origin}/logo_icon.svg).
...
/** Public URL for the header logo (email clients require absolute URLs). */
function getHeaderLogoUrl(appOrigin) {
  const custom = String(process.env.ADMIN_OUTREACH_LOGO_URL || "").trim();
  if (custom && isSafeHttpUrl(custom)) {
    return custom;
  }
  const base = String(appOrigin || "").replace(/\/$/, "");
  return `${base}/logo_icon.svg`;
}
...
export function buildAdminPlatformOutreachPlainText(opts) {
  ...
  t +=
    "You're receiving this because you have a Paidly account linked to this email address.\n";
  t += `Help and contact: ${c.appOrigin}\n`;
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;margin:0 auto;">
            ...
            <td style="background-color:${BRAND_HEADER_BG};border-radius:12px 12px 0 0;border-top:4px solid ${BRAND_ACCENT};padding:28px 24px 24px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  ...
                  <td valign="top" align="right" style="padding:0 0 0 16px;width:1%;white-space:nowrap;">
                    <a href="${attrEscape(loginUrl)}" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">
                      <img src="${attrEscape(headerLogoUrl)}" width="52" height="52" alt="Paidly" border="0" style="display:block;height:52px;width:52px;max-width:52px;border:0;outline:none;" />
                    </a>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color:${CARD_BG};padding:28px 24px 8px;font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
              ...
              <p style="margin:16px 0 0;font-size:13px;line-height:1.5;color:${MUTED};">
                ...
                <a href="${attrEscape(appOrigin)}" target="_blank" rel="noopener noreferrer" style="color:#2563eb;text-decoration:underline;">${escapeHtml(appOrigin.replace(/^https?:\/\//, ""))}</a>
              </p>
            </td>
          </tr>
          ${
          ...
          <tr>
            <td style="background-color:${CARD_BG};border-radius:0 0 12px 12px;padding:20px 24px 28px;font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:11px;line-height:1.5;color:#94a3b8;text-align:center;border-top:1px solid #e2e8f0;">
              You're receiving this because you have a Paidly account linked to this email address.<br/>
