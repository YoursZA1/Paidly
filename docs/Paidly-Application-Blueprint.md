...
**POS staff invite (till-only link):** From the **front-facing till** (`/POS`), a manager or admin (`manage_employees`) can **Invite staff**. That uses the existing company invite API (`POST /api/company/invite`) — not a second till login. The invite is always RBAC `employee` plus `memberships.job_function = pos` (`source = pos`). Optional **POS location** is `company_invites.register_id` → `pos_registers` (a till, not a store table). Cashiers cannot invite others. The shareable link is **`/pos/invite/:token`** (legacy `/invite?token=…&next=POS` still validates). Accepting (`accept_company_invite_token`) copies `job_function` onto `memberships` and may assign `pos_registers.assigned_staff_id`. POS-only staff land on `/POS`; the till back-arrow is Sign out. Settings → Team can still pick job function **POS only**. Migration: `supabase/migrations/20260828290000_pos_staff_invite.sql`.
...
- **Native POS till** (`/POS` or `/pos`): a **front-facing till** (not the admin dashboard). Layout skips sidebar/header/footer; the page is catalog + cart + pay. Staff open it from back-office **POS** nav (after Products) when inventory and `pos_access` allow — the till has no app chrome of its own. Search is scan-first: USB/Bluetooth wedge or camera barcode → catalog lookup (name, SKU / product code, barcode) → cart. **POS Customer is optional** (**Walk-in Customer** default). Checkout is **Sell → Pay**; attaching a customer is not a required step. The till selector lists only **POS customers** (`clients.pos_enabled`) — walk-in, POS-created, or explicitly enabled — never the general Paidly client database. New POS Customer collects name + optional phone. Back office remains the master for the full CRM.
