...
Do not create a second tenant system. `CompanyContext` remains org RBAC. Active brand is a **UI default for new documents** (per-org localStorage); changing it must not rewrite existing invoices **and must not change which products a POS till can sell**. Quotes have no `company_id` column (name/logo snapshot only). Payslips stay organization-profile scoped. **Business Logo** is `profiles.logo_url` (documents + profile). **POS Logo** is optional `profiles.pos_logo_url`; the till uses `pos_logo_url || logo_url` and never writes document branding. See `docs/MULTIBRAND_COMPANIES.md`.
