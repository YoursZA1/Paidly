...
- **Portals:** One **Workforce** sidebar item. Employee (`/Workforce` self-service), Manager (`/Workforce/manager` tabs: team / leave / calendar / me, direct reports only, no salary), and HR (`/Workforce` overview + `/Employees` + `/employees/:id` + `/Leave`; Payroll child only with `manage_payroll`) are permission views of the same `memberships` / `leave_requests` / `pay_runs` rows. Line-manager `memberships` SELECT is self + `manager_membership_id = caller`; admin / HR / finance stay org-wide. Compensation is omitted from `/api/company/employees` unless `manage_payroll` or the row is the caller. Settings → Team stays invites and roles.
