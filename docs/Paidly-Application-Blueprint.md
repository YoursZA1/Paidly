...
- **Provisioning:** `provisionEmployeeWorkforce()` (idempotent) auto-creates payroll profile + leave balances (year-to-date accrual) + attendance profile + notification-prefs stub + audit on `employee.created`. `/api/company/employees` on the existing company function. Cron `workforce-events` retries failed/pending rows older than 15s. Invite accept (`accept_company_invite_token`) links `company_invites.membership_id` onto the pre-created membership (`user_id` was null) and emits `employee.portal.activated`. POS till invites stay on the existing pass path and do not use that link. Open pay runs pick up new hires via `syncPayRunEmployees` (calculate + refresh). Standalone payslip compose persists `membership_id` and copies a processed pay-run item when the period matches.
...
**Migration status:** `supabase/migrations/20260902120000_payroll_engine_and_leave_ledger.sql`, `supabase/migrations/20260905120000_workforce_engine_core.sql`, `supabase/migrations/20260907200000_workforce_canonical_employee_identity.sql`, `supabase/migrations/20260908120000_workforce_identity_integrity.sql`.
