...
- **Portals:** Employee (`MyPayroll` + apply leave), Manager (`/ManagerPortal`, direct reports only, no salary), and HR (`/Employees` + `/employees/:id` profile + `/Leave` + `/Payroll`) are role views of the same `memberships` / `leave_requests` / `pay_runs` rows. Compensation is omitted from `/api/company/employees` unless `manage_payroll` or the row is the caller. Settings → Team stays invites and roles.
