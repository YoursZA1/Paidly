...
**POS staff invite (till-only link + code):** From the **front-facing till** (`/POS`), a manager or admin (`manage_employees`) can **Invite staff**. That uses the existing company invite API (`POST /api/company/invite`) — not a second till login. The invite is always RBAC `employee` plus `memberships.job_function = pos` (`source = pos`). **Till is required** (`company_invites.register_id` → `pos_registers`). A unique short till code (e.g. `7K4M-X92Q`) is generated with `crypto.randomInt`, **hashed (SHA-256)** into `invite_code_hash`, and shown once with `/pos/invite/:code`. Cashiers can also type the code at `/pos/join`. Validate through rate-limited `GET /api/company/invite/validate` (anon RPC is revoked). Accepting copies `job_function` and `memberships.pos_register_id`; POS-only staff cannot select another till. Revoke sets `status = revoked` + `revoked_at`. Migration: `supabase/migrations/20260831150000_pos_till_invite_code.sql`.
