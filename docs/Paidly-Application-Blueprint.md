...
- **Leave approval:** Apply holds pending days only. Assigned manager (or HR override) decides. Email uses hashed single-use tokens (`leave_approval_tokens`) via `/api/public-leave` rewritten onto `api/public-share`. The token page cannot see payroll or other employees. Approved unpaid leave is input to `calculatePayroll`; paid leave is not a salary formula in the portal.
