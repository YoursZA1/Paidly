...
| Invoice | `invoices` |
| Quote | `quotes` |
| Payslip | `payslips` (issued pay statement; generated from a pay run, not the employee source of truth) |
| Recurring invoice | `recurring_invoices` |
| Payroll run | `pay_runs` + `pay_run_items` + `payroll_profiles` (identity remains `memberships`) |
| Leave | `leave_types`, `leave_balances`, `leave_transactions`, `leave_requests` |
| Other business documents (expenses, contracts, …) | `documents` (+ `document_items`, `document_events`) |
...
- **Portals:** One **Workforce** sidebar item (not a second HR product). Org owner: full Paidly + Workforce HR home `/Dashboard`. Invited HR/admin: Workforce only (`/Workforce`). Finance manager: `/Workforce/payroll`. Line manager: `/Workforce/manager?tab=overview` (Overview / My team / Leave / Calendar / Me; direct reports only; no salary; no org HR children). Employee: `/Workforce` self-service home (emp no, job, department, leave balances, latest payslip, upcoming leave) plus own leave/payslips/profile. **Employee Portal Access ≠ POS Access** — same `memberships.id` identity, separate experiences. Portal: invite → create password → `/Workforce` (profile, payslips, leave, documents). POS (when enabled): `/pos` (or till URL) identifies the employee/till; **POS PIN authenticates** before Start/Resume Shift — never treat the POS link or till code as login, and never route POS into the Employee Portal. Role=`employee` without POS enablement (`membershipCanEnterPos` / `membershipIsPosEnabled`) cannot use `/pos` even though the generic RBAC matrix includes `pos_access`. Admins see separate **Employee Portal** and **POS Access** panels on Employee Overview (portal status + Copy Portal/Activation Link; POS Enabled + Assigned Till + Copy POS Link; POS PIN + Reset). Portal statuses: `not_activated` | `invitation_sent` | `activated` | `expired` | `revoked`. Existing `/pos/invite/...` device activation remains. POS PIN is membership-scoped and distinct from the Paidly Auth password. Employees and line managers do not see Dashboard, commercial Documents, Calendar, or Messages in the sidebar. Workforce Documents is not Document Engine — employee files stay on the profile Documents tab (`membership_id`). HR overview KPIs include total/active, pending invitations (`company_invites` when `manage_employees`), on leave today, upcoming leave, and new hires; payroll money KPIs require `manage_payroll`. Directory `leave_status` is derived from `leave_requests`, not stored. Employee profile **Payroll** tab is `manage_payroll` only. Canonical child URLs: `/Workforce/employees`, `/Workforce/payroll`; `/Employees` and `/Payroll` remain aliases. `/employee` redirects to `/Workforce`. Line-manager `memberships` SELECT is self + `manager_membership_id = caller`; admin / HR / finance stay org-wide. Compensation is omitted from `/api/company/employees` unless `manage_payroll` or the row is the caller. Settings → Team stays invites and roles.
- **Lifecycle:** Deactivate is a soft status (`employment_status = inactive` + `disabled_at`) on the same `memberships` row. Historical payslips, pay runs, leave, documents, audit, and `manager_membership_id` pointers stay. Inactive people cannot apply for leave, approve leave, or receive new reports. HR/admin bulk-reassign those reports onto an active manager. Directory “Needs attention” covers missing manager/department/payroll/HR fields, **incomplete pay rate** (live `payroll_profiles` salary/rate is 0 — not a suppressed validation), and **Manager inactive — reassignment required**. Mutations stay on `/api/company/employees` (existing Hobby company function) with `workforce_audit_logs`.
- **Directory performance:** `/api/company/employees` list is paginated (default 50) and does not hydrate payslips, leave balances, or attendance. Pickers (`listWorkforceEmployees`, Create Payslip) use `page_all=1` and unwrap `data.items` — they must not treat the paginated object as an empty array. `GET ?id=` loads one membership. `include=profile` or `sections=` lazy-loads profile tabs. `workforce-summary` uses the lite roster plus COUNT queries, not a full related-table join. Manager pickers use `eligible_managers=1`, not the full roster. Leave employee dropdowns batch-read payroll profiles (no GET-time provision).
- **Leave approval:** Apply holds pending days only. Assigned **active** manager (or HR override) decides. Inactive managers cannot decide, including email tokens. Email uses hashed single-use tokens (`leave_approval_tokens`) via `/api/public-leave` rewritten onto `api/public-share`. The token page cannot see payroll or other employees. Approved unpaid leave is input to `calculatePayroll`; paid leave is not a salary formula in the portal.
- **Provisioning:** `provisionEmployeeWorkforce()` (idempotent) auto-creates payroll profile + leave balances (year-to-date accrual) + attendance profile + `workforce_notification_prefs` + audit on `employee.created`. `/api/company/employees` on the existing company function. Cron retries pending/failed workforce events with the daily payment-reminders job (and `/api/cron/workforce-events`). Invite accept (`accept_company_invite_token`) links `company_invites.membership_id` onto the pre-created membership (`user_id` was null) and emits `employee.portal.activated`. POS till invites stay on the existing pass path and do not use that link. Open pay runs pick up new hires via `syncPayRunEmployees` (calculate + refresh). Pay-run finalize generates locked payslips and emails them; standalone compose remains for out-of-band drafts and must attach `membership_id`.
...
- **Locking:** Finalized pay runs and locked payslips cannot be silently rewritten. Corrections use an adjustment pay run. The shared `prevent_locked_payroll_mutation` trigger must not read `OLD.locked` on `pay_runs` / `pay_run_items` (those tables have no such column — recalculate would fail). Open-run calculate claims the `pay_runs` row (`claim_pay_run_for_calculate` / `commit_pay_run_calculate`) and retries if overlapping leave changes mid-calc. Leave decide locks overlapping open pay runs and the leave balance (`lock_open_pay_runs_for_leave`). Statutory org rates are append-only versions (`effective_from`); do not UPDATE `value` in place. Daily `payment-reminders` cron drafts outstanding adjustment runs, nags payroll admins after 24h, and alerts on unusual public payslip access.
- **Leave:** Server-side working-day counts (`Africa/Johannesburg`), overlap checks, and a leave ledger (`leave_transactions`). Annual leave accrues from the employment start date (BCEA 21 days, monthly). There is no 3-month waiting period. Approvals update balances transactionally via `/api/leave` and emit `employee.leave_approved`. Leave rows also store `employee_id → memberships.id`.
- **RBAC:** Existing company permissions. Manager + `job_function=hr` gains employee/leave admin; manager + `finance` gains payroll. POS-only staff cannot administer payroll or workforce. Line-manager `memberships` SELECT is self + direct reports (`manager_membership_id`); admin / HR / finance stay org-wide (`can_see_org_workforce`). Do not change `is_company_manager_for_org` (leave and POS still use it). Payslip compensation RLS (`can_read_payslip_row`) still hides other people’s rows unless payroll/admin. Payslip self-service matches `employee_user_id`, `membership_id`, or employee email.
- **Delivery:** Issued payslips stay on `payslips` with authenticated / email-gated public share tokens. Email uses the existing Resend path. Canonical issue path is pay-run finalize (payslip copies the payroll item, `status=published`, `locked=true`, plus `employer_snapshot` from the live org profile). Historical locked rows stored as `draft` display as **Published**. Standalone compose still exists for drafts and must attach `membership_id` (and may store `employer_snapshot`). `/api/payroll/payslips/:id/publish` and `/send` stay on the existing payroll rewrite.
- **Payroll compliance reports:** Workforce → Reports (`/Workforce/reports`) exposes headcount KPIs to team viewers. Payroll admins also get historical **UIF / PAYE / Payroll Summary / Net Pay Register** via `GET /api/payroll/reports` over finalized `pay_run_items` (no recalculation). Employer PAYE/UIF/SDL refs live on `organizations.payroll_settings` (Settings → Employer payroll details; also `GET/POST /api/payroll/employer-settings`). Net Pay Register is display + CSV only in this phase — bank-file import and financial-dashboard payroll wiring are later.
- **Next (deferred):** Organisation organogram (visualize existing `manager_membership_id` / department — no second hierarchy table); People Calendar (birthdays need `date_of_birth`; work anniversaries from `employment_start_date`); Net Pay → mark-paid confirmation UX; financial dashboard Revenue → Expenses → Payroll.
- **Attendance:** `attendance_profiles` is a derived 1:1 stub provisioned with the employee. Time-and-attendance product features subscribe later without creating a second person. The Attendance page lists that stub; it does not add clock-in.

**Migration status:** `supabase/migrations/20260902120000_payroll_engine_and_leave_ledger.sql`, `supabase/migrations/20260905120000_workforce_engine_core.sql`, `supabase/migrations/20260907200000_workforce_canonical_employee_identity.sql`, `supabase/migrations/20260908120000_workforce_identity_integrity.sql`, `supabase/migrations/20260910120000_workforce_profile_single_source.sql`, `supabase/migrations/20260910130000_workforce_integrity_tighten.sql`, `supabase/migrations/20260910140000_workforce_leave_tokens_and_prefs.sql`, `supabase/migrations/20260910150000_workforce_payrun_membership_unique.sql`, `supabase/migrations/20260912120000_leave_transactions_employee_not_null.sql`, `supabase/migrations/20260912130000_payslips_membership_required.sql`, `supabase/migrations/20260912140000_payslip_compensation_rls.sql`, `supabase/migrations/20260914120000_memberships_manager_select_direct_reports.sql`, `supabase/migrations/20260915120000_workforce_payrun_leave_locks.sql`, `supabase/migrations/20260915130000_payroll_statutory_append_only.sql`, `supabase/migrations/20260915140000_workforce_directory_indexes.sql`, `supabase/migrations/20260915150000_fix_prevent_locked_payroll_mutation.sql`, `supabase/migrations/20260918120000_payslips_employer_snapshot.sql`.
...
**POS staff invite (email activates the till):** From the **front-facing till** (`/POS`) or **Settings → Team Members**, a manager or admin (`manage_employees`) can invite POS staff. That uses the existing company invite API (`POST /api/company/invite`) — not a second invite table. Access role and function stay separate: **Employee + Function POS only** is a POS invite (`source = pos`, RBAC stays `employee`, `job_function = pos`). Till is required (`company_invites.register_id` → `pos_registers`; no `store_id`). The invitation row is **persisted first**; email is a delivery method. The staff CTA is **Open Paidly POS**. The share URL is always reconstructed from the stored hex `token` using `PUBLIC_APP_URL` / `PUBLIC_APP_ORIGIN` / `APP_URL` (production never emits localhost or `*.vercel.app`): `/pos/invite/{secure-token}`. Staff do **not** type a code after clicking the email.

That page validates the invitation server-side, then **Continue to POS** consumes it into a **scoped POS access-pass session** (`pos_access_sessions`, HttpOnly cookie `paidly_pos_access` + optional `Authorization: Bearer pos.<token>`). Employees **without a Paidly account are not sent through Signup**. Existing Paidly users are linked (membership) when the invited email matches, still without onboarding. The pass is not a Paidly Auth session: it cannot open Settings, Billing, Invoices, Reports, Admin, or other businesses. Permissions still come from the invite’s server-side role/`job_function` (POS-only grants), not from the browser. After acceptance the employee lands on **Paidly POS → assigned till → Start Shift** (or Resume Shift if that till already has an open shift). The invite token is discarded; returning staff reuse the POS session until it expires. Expired/revoked/consumed invites never open POS. Direct `/pos` still requires Auth `pos_access` **or** a valid POS access session. Standard company invitations (`/invite/:token`) are unchanged. APIs: `POST /api/pos/invite-activate`, `GET /api/pos/access`, `POST /api/pos/access-end` on the existing POS function (no 13th Hobby function). Migration: `supabase/migrations/20260904120000_pos_access_sessions.sql`. A unique short till code remains **backup / device activation only** (`/pos/join`). Old emails that still contain `/pos/invite/{short-code}` continue to validate (token **or** code hash). Admins can **copy / resend / revoke** from Invitation management; resend **reuses** a still-valid pending token and only rotates after expiry. Hobby production calls one-segment aliases (`POST /api/company/invite-resend?id=`, `DELETE /api/company/invite-by-id?id=`, `GET /api/company/invite-validate`) on the existing company function — nested `/invites/:id/resend` never reaches that handler. `email_sent_at` is set only after Resend (or SMTP) confirms submission. Wrong-account accept (signed in as a different email) returns `email_mismatch`. POS cashiers skip company/plan signup onboarding. Migration: `supabase/migrations/20260831150000_pos_till_invite_code.sql`, `supabase/migrations/20260831170000_company_invite_email_sent.sql`.

**POS URL / access:** The dedicated POS URL is **https://www.paidly.co.za/pos** (`/pos`; `/POS` redirects here). **`/pos` is not a credential** and does not skip Auth. If a Paidly Auth session already has `pos_access`, that existing session opens the till. Otherwise the page shows a **POS sign-in / device activation** screen (same Auth + memberships — not a second POS login). A till-specific bookmark is **https://www.paidly.co.za/pos/till/{till-id}** (`pos_registers.id`). Knowing that path does not grant access: guests still sign in; POS-only staff can only open their assigned till; checkout still uses server RBAC. Generate till links from **Settings → Registers**. After an invitation is accepted, staff land on that till URL → Start Shift. Primary sidebar Overview order: **Dashboard → Invoices → Quotes → Clients → Products → POS** (then Documents, Purchase Orders, Reports, Settings). The **POS** item is shown only when the org **opted into POS** (`organizations.business_type` is `retail` or `mixed`), the plan includes the `pos` feature (Business+), **and** the user has `pos_access`. Service orgs (and unset type) never see POS in the sidebar — it is not an upgrade teaser. Company members also get **POS** as a Me-section workspace link when those same gates pass. Dashboard **Open POS** stays visible even when there are no sales today. Mobile bottom nav stays Home / Invoices / FAB / Clients / Menu; POS lives in the sidebar Menu drawer. `canShowPosNav` (`src/lib/posNavAccess.js`) is the single visibility helper.
...
**POS acceptance tests (Task 28):** The till contract is `tests/unit/posTask28Acceptance.test.js` (TESTs 1–15: open till, search, cart, quantity, cash change, digital paid vs not paid, inventory commit point, receipt, attach customer, register brand, RBAC, persistence vs held cart, tenant isolation, duplicate payment). Supporting modules: `tests/unit/posSaleProcessor.test.js` (webhook idempotency), `tests/pos.spec.ts` (guest cannot open `/pos` or `/pos/till/{id}` without Auth; entitled session may exercise the till). Do not treat a held `sessionStorage` cart as a sale. Digital checkout writes `pos_sales_events` only when `posSaleCompletesWhenPaid` (`payment_intents.status = paid`).
