...
- **Delivery:** Issued payslips stay on `payslips` with authenticated / email-gated public share tokens. Email uses the existing Resend path. Canonical issue path is pay-run finalize (payslip copies the payroll item, `status=published`, `locked=true`). Historical locked rows stored as `draft` display as **Published**. Standalone compose still exists for drafts and must attach `membership_id`. `/api/payroll/payslips/:id/publish` and `/send` stay on the existing payroll rewrite.
