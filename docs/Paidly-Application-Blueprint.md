...
- **Locking:** Finalized pay runs and locked payslips cannot be silently rewritten. Corrections use an adjustment pay run. Open-run calculate claims the `pay_runs` row (`claim_pay_run_for_calculate` / `commit_pay_run_calculate`) and retries if overlapping leave changes mid-calc. Leave decide locks overlapping open pay runs and the leave balance (`lock_open_pay_runs_for_leave`). Statutory org rates are append-only versions (`effective_from`); do not UPDATE `value` in place. Daily `payment-reminders` cron drafts outstanding adjustment runs, nags payroll admins after 24h, and alerts on unusual public payslip access.
