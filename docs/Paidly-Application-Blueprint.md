...
- **SaaS subscription SoR (billing v2):** `plans` catalog (`amount`, `billing_cycle`, `payfast_item_name`, `features`, `active` — **not hardcoded in React**) + `subscriptions` core (`company_id`, `plan_id`, PayFast ids, period bounds, `cancelled_at`, `created_by`; status allow-list only: `pending` \| `processing` \| `active` \| `past_due` \| `failed` \| `cancelled` \| `expired` \| `suspended` \| `trialing`) + append-only `payment_history` (never delete; statuses: `pending` \| `completed` \| `failed` \| `cancelled` \| `refunded`) + `subscription_events` (Event Timeline: Created → Redirected → ITN Received → Verified → Activated → Renewed → Cancelled; allow-listed `event_type` only) + `payfast_itn_logs` (raw ITN + verification flags; never trust until `verified`) + `subscription_invoices` (SaaS tax invoices after successful payment; statuses: `draft` \| `paid` \| `void` \| `cancelled` — distinct from Document Engine `invoices`) + `webhook_logs` (provider / headers / body / response / status_code / duration for debugging). Activation only after verified PayFast ITN (never from the SPA). RLS: `company_id = current_company_id()` for members; billing admins see all; `payfast_itn_logs` never exposed to clients; service_role-only writes on `payment_history` / `subscription_events` / `payfast_itn_logs` / `webhook_logs`. **APIs:** `POST /api/subscriptions/create`, `GET …/status|current`, `POST …/cancel`, `POST /api/payfast/itn` (save raw → signature → IP → PayFast VALID → merchant/amount/subscription → dedupe → update + `payment_history` + events → 200), `POST /api/internal/activate|expire`, `GET /api/admin/subscriptions|revenue|failed-payments` (`server/src/billing/*`). Schema: `docs/SUBSCRIPTION_BILLING_SCHEMA.md`, migration `20260715170000_subscription_billing_v2_schema.sql`. Profiles (`plan` / `subscription_status` / `is_pro`) remain a cache.
