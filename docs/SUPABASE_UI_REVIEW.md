...
|----------|------------------|------------|--------|-------|
| **uploadLogo** | logoUpload.js | Settings (Save with logo), SetupWizard.uploadLogo, OrgBrandsSettings | Stored path in logo_url, form save | Toast (Settings), or error in wizard |
| **upload** (paidly / private buckets) | logoUpload.js, customClient Core.UploadFile/UploadPrivateFile | Settings logo, onboarding SetupWizard, expense/bank attachments | Path or URL returned, profile updated | Throw → toast or catch in caller |
...
| **createSignedUrl / getPublicUrl** | AssetService, LogoImage, customClient buildFileUrl | Logo display, after upload | Image displays | Fallback to public URL or placeholder, console.warn |
