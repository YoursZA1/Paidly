-- Quote → invoice traceability on specialised tables (not documents hub).
-- One invoice per quote. Converted quotes stay immutable.

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS source_quote_id uuid;

ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS invoices_source_quote_id_fkey;
ALTER TABLE public.invoices
  ADD CONSTRAINT invoices_source_quote_id_fkey
  FOREIGN KEY (source_quote_id) REFERENCES public.quotes(id) ON DELETE RESTRICT;

CREATE UNIQUE INDEX IF NOT EXISTS idx_invoices_source_quote_id
...
COMMENT ON COLUMN public.invoices.source_quote_id IS
  'Specialised quote this invoice was converted from. Hub documents.source_quote_id is not used.';

COMMENT ON COLUMN public.quotes.converted_at IS
  'Set when this quote was converted to an invoice. Quote remains historical evidence.';

CREATE OR REPLACE FUNCTION public.enforce_invoice_source_quote_immutable()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  IF TG_OP = 'UPDATE'
     AND OLD.source_quote_id IS NOT NULL
     AND NEW.source_quote_id IS DISTINCT FROM OLD.source_quote_id THEN
    RAISE EXCEPTION 'source_quote_id cannot be changed';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_invoices_source_quote_immutable ON public.invoices;
CREATE TRIGGER trg_invoices_source_quote_immutable
  BEFORE UPDATE ON public.invoices
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_invoice_source_quote_immutable();
...
DROP TRIGGER IF EXISTS trg_quotes_converted_immutable ON public.quotes;
CREATE TRIGGER trg_quotes_converted_immutable
  BEFORE UPDATE OR DELETE ON public.quotes
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_converted_quote_immutable();
...
BEGIN
  IF EXISTS (
    ...
    WHERE q.id = v_quote_id
      AND (q.status = 'converted' OR q.converted_at IS NOT NULL)
  ) THEN
    RAISE EXCEPTION 'converted quotes are immutable';
  END IF;
  IF TG_OP = 'DELETE' THEN
...
DROP TRIGGER IF EXISTS trg_quote_items_converted_immutable ON public.quote_items;
CREATE TRIGGER trg_quote_items_converted_immutable
  BEFORE INSERT OR UPDATE OR DELETE ON public.quote_items
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_converted_quote_items_immutable();
...
COMMENT ON FUNCTION public.convert_quote_to_invoice(uuid, jsonb) IS
  'Atomic quote→invoice conversion on specialised tables. Idempotent: returns the existing invoice when source_quote_id already exists.';
