...
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS barcode text;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS image_url text;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS stock_capacity numeric(12,2);
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS stock_quantity integer;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS cost_price numeric(12,2);
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS low_stock_threshold integer DEFAULT 5;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND indexname = 'idx_services_org_active_barcode_unique'
  ) THEN
    BEGIN
      EXECUTE $idx$
